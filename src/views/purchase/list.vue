	...
	<v-row>
		...
		<v-col class="text-end">
			...
			<v-btn
				...
				@click="
					() => {
						SkuStore.setSchema(ENUM_ORDER_TYPE.PURCHASE);
		...
		<v-expansion-panel bg-color="grey-lighten-4">
			<v-expansion-panel-title class="py-0">
				<v-row no-gutters>
							...
							<template v-slot:activator="{ props }">
								...
								>
									<template v-slot:loader>
										<v-progress-linear v-if="SubjectContactStore.loadding" indeterminate color="primary"></v-progress-linear>
									</template>
								</v-text-field>
							</template>
							<v-list>
					...
					<v-col cols="1" class="text-right pt-5">已付款</v-col>
		...
		<v-expansion-panel v-for="order in OrderStore.orderList">
			<v-expansion-panel-title class="py-0">
				<v-row no-gutters>
					...
					<v-col cols="1" class="line-28 pl-2 text-grey-lighten-2">{{ (order.amountCertificationRest / 100).toFixed(2) }}</v-col>
					<v-col>
						<v-btn size="small" class="mr-1" :color="order.managerId ? '' : 'primary'" @click.stop="setManager(order)">
							<v-icon icon="mdi-check" color="primary" v-if="order.managerId"></v-icon>
							<span v-else>复核</span>
						</v-btn>
						<v-btn size="small" class="mr-1" @click.stop="setAccountor(order)" :color="order.accounterId ? '' : 'teal'">
							<v-icon icon="mdi-check" color="primary" v-if="order.accounterId"></v-icon>
							<span v-else>结清</span>
						</v-btn>
						<v-btn
							size="small"
							...
							@click.stop="postOrderGetIn(order, order.joinSku)"
						>
		...
		<v-card>
			...
			<v-card-text>
				...
				<v-card class="d-flex pa-2">
					...
					<v-btn v-if="SubjectContactStore.subjectContactPicked._id" color="primary" class="ml-2" @click="putOrder()">
...
const postOrderGetIn = async (entity: Order, skuList?: SkuJoined[]) => {
	try {
		OrderStore.setSchema(ENUM_ORDER_TYPE.GETIN);
		const schema = OrderStore.getSchema(ENUM_ORDER_TYPE.GETIN);
		...
		router.push("/view/warehouse/getin");
	} catch (error) {
		SnackbarStore.setFail((error as Error).message);
	}
};
