<template>
    ...
    <div class="row">
        <q-card
            ...
            :class="house._id === WarehouseStore.picked?._id ? 'w-325' : ''"
        >
            <q-card-section>
                <div class="text-h6 ellipsis" :class="house._id === WarehouseStore.picked?._id ? 'text-weight-bold' : 'text-grey'">
                    {{ house.name }}
                </div>
    ...
    <div class="row">
        <q-card
            v-for="house in WarehouseStore.list.filter((e) => e.isDisabled === true)"
            class="w-200 q-mr-md q-mb-md"
            :class="house._id === WarehouseStore.picked?._id ? 'w-325' : ''"
        >
            <q-card-section>
    ...
    <q-dialog v-model="dialogWarehouse" persistent>
        <q-card class="w-400">
            ...
            <q-card-section>
                <q-input filled label="仓库名称" class="q-mb-sm" v-model="WarehouseStore.editor.name">
                    <template v-slot:before>
                        <q-icon name="person" />
                    </template>
                </q-input>

                <q-input filled label="仓库地址" class="q-mb-sm" v-model="WarehouseStore.editor.address">
                    <template v-slot:before>
                        <q-icon name="event" />
                    </template>
                </q-input>
            ...
            <q-card-actions align="right">
                <q-btn
                    ...
                    @click="
                        async () => {
                            if (WarehouseStore.editor._id) await WarehouseStore.patch();
                            else await WarehouseStore.post();
                            dialogWarehouse = false;
                        }
                    "
                >
                    {{ WarehouseStore.editor._id ? "保存" : "新增" }}
                </q-btn>
            </q-card-actions>
        </q-card>
    </q-dialog>
...
import { getTimeGap } from "@/lib";
import { useNotifyStore } from "@/stores/quasar/notify";
import { useUserStore } from "@/stores/user/user";
import { useCorpStore } from "@/stores/brand/corp";
import { useWarehouseStore } from "@/stores/wmss/warehouse";
import { useBrandRoleStore } from "@/stores/brand/roled/role.js";
import { useAnalysisStore } from "@/stores/wmss/analysislysis.js";
