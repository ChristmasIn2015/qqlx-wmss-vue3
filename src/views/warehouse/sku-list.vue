<template>
		...
		>
			<template v-slot:top="props">
				<span class="text-grey">
					<q-carousel vertical animated infinite :autoplay="4000" navigation v-model="swiperIndex" height="20px">
			...
			<template v-slot:header="props">
				<q-tr>
					...
					<q-th
						...
						class="cursor-pointer"
						:class="{
							'text-indigo-14': SkuStore.sortKey === 'count',
						}"
					...
					<q-th key="_id" :props="props">当前状态</q-th>
				</q-tr>
			</template>
					...
					<q-td key="_id" :props="props" style="padding: 0 4px">
						<span v-if="props.row.isConfirmed">
							<span v-if="props.row.type === ENUM_ORDER.GETOUT">
								...
								<span v-else class="text-negative">已发货</span>
							</span>
							<span v-else-if="props.row.type === ENUM_ORDER.MATERIAL">
								<q-btn dense @click="SkuStore.patch([props.row])"> 取消 </q-btn>
							</span>
							<span v-else class="text-positive">已入库</span>
						</span>
						<span v-else>
				...
				<div class="q-pt-sm q-pb-xs">
					<span v-show="endIndex !== -1 && endIndex - startIndex >= 0">
						<span>
							...
							<a class="q-mx-sm text-negative text-body1 text-weight-bold">{{ skuPicking.pounds.toFixed(3) }}</a>
							<span>吨，</span>
						</span>
					</span>
	...
	<q-dialog v-model="skuGetInDialog" maximized transition-show="slide-up" transition-hide="slide-down">
		<q-card>
			...
			<q-card-section>
				<picker-sku-getin :name="skuGetInPicking?.name" :norm="skuGetInPicking?.norm" @pick="pickSkuGetIn" />
			</q-card-section>
		</q-card>
	</q-dialog>
</template>
...
const columns = ref([
	...
	{ name: "pounds", field: "pounds", label: "重量", style: cellStyle },
