		...
		<v-row>
			<v-col>
				<v-tabs bg-color="amber" v-model="tab">
					<v-tab value="create">创建加工单</v-tab>
					<v-tab value="list">加工单列表</v-tab>
				</v-tabs>
			</v-col>
		</v-row>
				...
				<v-row class="pa-2">
					...
					<v-col class="text-end">
						...
						<v-btn
							...
							@click="
								() => {
									SkuStore.setSchema(ENUM_ORDER_TYPE.PROCESS);
				...
				<v-btn
					...
					@click="
						() => {
							...
							SkuStore.setSchema();
							OrderStore.setSchema(ENUM_ORDER_TYPE.PROCESS);
...
const postOrder = async () => {
	try {
		OrderStore.setSchema(ENUM_ORDER_TYPE.PROCESS);
		const schema = OrderStore.getSchema(ENUM_ORDER_TYPE.PROCESS);
