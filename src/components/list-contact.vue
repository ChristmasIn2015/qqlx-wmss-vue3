<template>
    <q-table style="min-height: 700px; max-height: 700px" row-key="_id" dense
        :rows-per-page-options="[ContactStore.page.pageSize]" :rows="ContactStore.list" :columns="[
                ...
                <q-btn padding="xs" flat color="negative">
                    ...
                    <q-menu>
                        <q-list>
                            ...
                            <q-item clickable v-close-popup @click="
                                () => {
                                    ContactStore.search.type = ENUM_CONTACT.NONE;
                                    ContactStore.get(1);
                                }
                            ">
        ...
        <template v-slot:top="props">
            <div class="col q-mb-md">
                <q-input dense standout clearable color="primary" placeholder="请输入客户名称、联系方式、备注"
                    v-model="ContactStore.search.name" :loading="ContactStore.loadding" @blur="ContactStore.get(1)">
            ...
            <div class="col text-right q-mb-md">
                <q-btn class="q-ml-sm" push square label="最近删除" :color="ContactStore.search.isDisabled ? 'primary' : ''"
                    :text-color="ContactStore.search.isDisabled ? '' : 'black'" @click="
                        () => {
                ...
                <span v-if="ContactStore.listExcel.length > 0">
                    <q-btn class="q-ml-sm" push square :label="`保存 ${ContactStore.listExcel.length}项`" color="primary"
                        @click="() => ContactStore.post()" />
                    <q-btn class="q-ml-sm text-negative" push square label="清空"
                        @click="() => (ContactStore.listExcel = [])" />
                    <q-btn class="q-ml-sm" push square label="批量导入">
                        <q-menu>
                            <q-list>
                                ...
                                <q-item clickable>
                                    <q-item-section>
                                        <q-file dense borderless accept=".xlsx, .xls" v-model="filePicking" label="选择文件"
                ...
                <q-btn class="q-ml-sm" push square color="negative" label="继续添加"
                    @click="() => ContactStore.listExcel.push(ContactStore.getSchema())" />
            </div>
        </template>
        <template v-slot:top-row>
            <q-tr v-for="(schema, index) in ContactStore.listExcel">
                ...
                <q-td>
                    <q-input square filled v-model="schema.remark" dense clearable color="primary"
                        placeholder="请输入备注" />
                </q-td>
                <q-td>
                    <q-input square filled v-model="schema.address" dense clearable color="primary"
                        placeholder="请输入联系方式" />
                </q-td>
                <q-td>
                    <div class="row items-center">
                        ...
                        <q-space></q-space>
                        <q-btn icon="close" dense class="text-negative" flat
                            @click="() => ContactStore.listExcel.splice(index, 1)"> </q-btn>
                    </div>
                </q-td>
            </q-tr>
        </template>
        <template v-slot:body="props">
            <q-tr :props="props">
                ...
                <q-td key="timeCreateString" :props="props">
                    <div class="row items-center">
                        <span>
                            <q-chip color="primary" dense class="text-white" square>{{
                                MAP_ENUM_CONTACT.get(props.row.type)?.text }}</q-chip>
                        </span>
                        <q-space></q-space>
                        <q-btn dense icon="more_horiz" class="q-ml-sm" flat>
                            <q-menu anchor="top left">
                                <q-item clickable>
                                    <q-item-section v-close-popup @click="
                                        () => {
                                ...
                                <q-item clickable>
                                    <q-item-section class="text-negative" v-close-popup
                                        @click="ContactStore.delete(props.row._id)">
                                        {{ props.row.isDisabled ? "恢复" : "删除" }}
                                    </q-item-section>
                                </q-item>
                            </q-menu>
                        </q-btn>
                        <q-btn dense push square class="q-ml-sm" color="primary" v-close-popup
                            @click="$emit('pick', cloneDeep(props.row))"> 选择 </q-btn>
                    </div>
                </q-td>
            </q-tr>
        </template>
        <template v-slot:bottom="props">
            <q-pagination square size="16px" color="white" class="q-my-sm" text-color="black" active-color="primary"
                active-text-color="white" v-model="ContactStore.page.page" :max-pages="10"
    ...
    <q-dialog v-model="dialogPatch">
        <q-card class="w-400">
            ...
            <q-card-section>
                ...
                <q-select v-model="ContactStore.editor.type" filled map-options emit-value option-value="value"
                    :options="[...MAP_ENUM_CONTACT].map((e) => ({
                        label: e[1].text,
                        value: e[1].value,
                    }))
                        ">
