<template>
	<v-card>
		<v-table fixed-header height="450" density="compact">
			...
			<tbody>
				<tr v-for="(order, index) in OrderStore.orderListPicked">
					<td>{{ order.code }}</td>
					<td>{{ order.joinContact?.name }}</td>
					<td>{{ order.timeCreateString }}</td>
					<td>
						<table-input placeholder="请输入金额" v-model="order.amount"></table-input>
					</td>
					<td>
						<v-btn icon="mdi-close" variant="text" class="mr-n1" color="red" size="32" @click="OrderStore.orderListPicked.splice(index, 1)"></v-btn>
					</td>
				</tr>
			</tbody>
		</v-table>
