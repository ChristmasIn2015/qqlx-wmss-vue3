		...
		>
			<template v-slot:top="props">
				...
				<span class="text-grey">
					<q-carousel vertical animated infinite :autoplay="5000" navigation v-model="swiperIndex" height="18px">
						<q-carousel-slide :name="0" class="q-pa-none">
							从上到下，长按A商品至B商品，可以批量选中A-B之间的所有商品，并在右下角进行修改 1/3
						</q-carousel-slide>
						<q-carousel-slide :name="1" class="q-pa-none"> 搜索商品时，仅需要输入关键字，就会开始自动搜索 2/3</q-carousel-slide>
						<q-carousel-slide :name="2" class="q-pa-none"> 商品分类会依照更新时间从最后修改的分类开始排序 3/3</q-carousel-slide>
					</q-carousel>
				</span>
				<q-btn fab icon="settings" flat dense color="primary" @click="$router.push('/wmss/system/cabinet')"></q-btn>
			</template>
				...
				<q-tr :props="props">
					...
					<q-th key="name" :props="props" style="width: 188px">
						<q-input
							...
							@update:model-value="(value)=>{
								CabinetUnitStore.cabinetUnitSearch.name = value as string
								debounceInit()
							}"
						/>
					</q-th>
					...
					<q-th key="norm" :props="props" style="width: 188px">
						<q-input
							...
							@update:model-value="(value)=>{
								CabinetUnitStore.cabinetUnitSearch.norm = value as string
								debounceInit()
							}"
						/>
					</q-th>
				...
				>
					...
					<q-td key="countFinal" :props="props">
						<a class="cursor-pointer text-grey">
							...
							<span
								v-else
								:class="props.row.countFinal > 1 ? `text-${route.meta.color}` : ''"
